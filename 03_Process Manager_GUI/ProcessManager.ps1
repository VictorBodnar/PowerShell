...
Function GetProcesses{
    ...
    $Processes = Get-Process -ComputerName $ComputerName | Select Id,ProcessName,Handles,NPM,PM,WS,VM,CPU,Path
...
Function EndProcesses{
    ...
    $SelectedProcesses | ForEach-Object {
        ...
        if ($ComputerName = $env:COMPUTERNAME) {stop-process -Id $ProcessId -Confirm:$false -Force}
        else {Invoke-Command -ComputerName $ComputerName -scriptblock {param($ProcessId) stop-process -Id $ProcessId -Confirm:$false -Force} -ArgumentList $ProcessId}
...
$button_GetProcess = New-Object System.Windows.Forms.Button
    ...
    $button_GetProcess.Font = "Segoe UI, 10"
    $button_GetProcess.Text = “Refresh Process List”
    $button_GetProcess.Add_Click({$ComputerName = $textbox_ComputerName.Text;GetProcesses})
...
$button_EndProcess = New-Object System.Windows.Forms.Button
    ...
    $button_EndProcess.Font = "Segoe UI, 10"
    $button_EndProcess.Text = “End Selected Process(es)”
    $button_EndProcess.Add_Click({$ComputerName = $textbox_ComputerName.Text;`
                                  EndProcesses})
        $Form_HelloWorld.Controls.Add($button_EndProcess)

$textbox_ComputerName = New-Object System.Windows.Forms.TextBox
    $textbox_ComputerName.Location = New-Object System.Drawing.Size(8,8)
    $textbox_ComputerName.Size = New-Object System.Drawing.Size(100,32)
    $textbox_ComputerName.Text = $ComputerName
    $textbox_ComputerName.Font = "Segoe UI, 10"
    $textbox_ComputerName.TextAlign = "Center"
        $Form_HelloWorld.Controls.Add($textbox_ComputerName)
